...
export default function StatsScreen({ navigation }) {
    ...
    try {
      ...
      for (const day of monthDays) {
        ...
        const checkOutTime = checkOutLog
          ? new Date(checkOutLog.timestamp)
          : null;
        ...
        if (checkInTime && checkOutTime && currentShift) {
          ...
          const { regularHours: regHrs, overtimeHours } = calculateWorkHours(
            checkInTime,
            checkOutTime,
            currentShift
          );
        ...
        stats[dateString] = {
          ...
          hasData:
            checkInTime !== null ||
            checkOutTime !== null ||
            (workStatus && workStatus.status !== "not_updated"),
        };
      }
  ...
  return (
    ...
    >
      ...
      <View style={styles.monthSelector}>
        ...
        <Text style={[styles.monthName, { color: darkMode ? "#fff" : "#000" }]}>
          {monthName}
        </Text>
      ...
      {isLoading ? (
        <View style={styles.loadingContainer}>
          ...
          <Text
            style={[styles.loadingText, { color: darkMode ? "#fff" : "#000" }]}
          >
  ...
  function calculateGrandTotal(stats) {
    ...
    Object.values(stats).forEach((dayStat) => {
      if (dayStat.hasData) {
        if (dayStat.regularHours !== "-")
          total += Number.parseFloat(dayStat.regularHours);
