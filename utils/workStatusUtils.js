...
export const calculateWorkStatus = async (date) => {
  try {
    ...
    if (checkInLog && checkOutLog) {
      ...
      const isNightShiftWork = isNightShift(
      ...
      const workStatusDetails = determineWorkStatus(
        checkInTime,
    ...
    switch (logType) {
      ...
      case "check_out": {
        ...
        if (checkInLog && checkOutLog && activeShift) {
          ...
          // Calculate work time
          const { totalHours } = calculateTotalWorkTime(
            checkInTime,
            adjustedCheckOutTime
          );
          ...
          if (activeShift) {
            ...
            if (adjustedCheckOutTime > officeEndTime) {
              ...
              const overtimeHours =
                Math.round((overtimeMs / (1000 * 60 * 60)) * 10) / 10;
