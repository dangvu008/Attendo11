...
export const STATUS_TYPES = {
  ...
  LEAVE: "leave", // 📩 P - Nghỉ Phép - Approved leave
  SICK: "sick", // 🛌 B - Nghỉ Bệnh - Sick leave
  HOLIDAY: "holiday", // 🎌 H - Nghỉ Lễ - Public holiday
  ABSENT: "absent", // ❌ X - Vắng Không Lý Do - Unauthorized absence
  LATE_ARRIVAL: "late_arrival", // RV - Vào Muộn - Late arrival
  EARLY_DEPARTURE: "early_departure", // RV - Ra Sớm - Early departure
  LATE_AND_EARLY: "late_and_early", // RV - Vào Muộn & Ra Sớm - Both late and early
...
export const determineStatus = (logs, shift) => {
  ...
  const isLateArrival =
    checkInTime > new Date(shiftStartTime.getTime() + 5 * 60 * 1000);

  // Check for early departure (left more than 5 minutes early)
  const isEarlyDeparture =
    checkOutTime < new Date(shiftEndTime.getTime() - 5 * 60 * 1000);

  // Check for overtime (more than 30 minutes after end time)
  const isOvertime =
    checkOutTime > new Date(shiftEndTime.getTime() + 30 * 60 * 1000);
...
export const formatStatusDetails = (logs, status, shift) => {
  ...
  if (checkInLog) {
    const checkInTime = new Date(checkInLog.timestamp);
    details.push(
      `Giờ vào: ${checkInTime.toLocaleTimeString("vi-VN", {
        hour: "2-digit",
        minute: "2-digit",
      })}`
    );
  ...
  if (checkOutLog) {
    const checkOutTime = new Date(checkOutLog.timestamp);
    details.push(
      `Giờ ra: ${checkOutTime.toLocaleTimeString("vi-VN", {
        hour: "2-digit",
        minute: "2-digit",
      })}`
    );
...
export const getStatusOptions = () => [
  ...
  { value: STATUS_TYPES.LEAVE, label: "Nghỉ Phép (📩 P)" },
  { value: STATUS_TYPES.SICK, label: "Nghỉ Bệnh (🛌 B)" },
  { value: STATUS_TYPES.HOLIDAY, label: "Nghỉ Lễ (🎌 H)" },
  { value: STATUS_TYPES.ABSENT, label: "Vắng Không Lý Do (❌ X)" },
  { value: STATUS_TYPES.LATE_ARRIVAL, label: "Vào Muộn (RV)" },
  { value: STATUS_TYPES.EARLY_DEPARTURE, label: "Ra Sớm (RV)" },
];
