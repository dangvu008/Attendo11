...
export const AppProvider = ({ children }) => {
  ...
  // Set active shift
  const handleSetCurrentShift = async (shiftId) => {
    try {
      console.log("Setting current shift with ID:", shiftId);

      // Call database function to set active shift
      const result = await setActiveShift(shiftId);

      if (result) {
        // Get the updated active shift with all fields
        const shift = await getActiveShift();

        // Ensure daysApplied exists
        if (shift && !shift.daysApplied) {
          shift.daysApplied = [false, false, false, false, false, false, false];
          console.log("Added missing daysApplied to shift:", shift.id);
        }
