...
export default function WeeklyStatusGrid({ darkMode }) {
  const { currentShift, t } = useContext(AppContext);
  ...
  const renderDetailsModal = () => {
    ...
    const dateString = formatDate(selectedDate);
    const dayStatus = weeklyStatus[dateString] || {
      status: "not_updated",
      logs: [],
    };
    const { status, logs, remarks } = dayStatus;

    const { icon, iconType, label, color, description, symbol } =
        ...
        >
          ...
          >
            <View style={styles.modalHeader}>
              ...
              <View
                style={[styles.statusIconContainer, { backgroundColor: color }]}
              >
            ...
            <View style={styles.detailsContainer}>
              <View style={styles.statusLabelContainer}>
                ...
                {symbol && (
                  <Text style={[styles.statusSymbolLabel, { color: color }]}>
                    ({symbol})
                  </Text>
                )}
              </View>
      ...
      <View style={styles.headerRow}>
        {weekDays.map((day, index) => (
          <View key={index} style={styles.dayHeader}>
            <Text
              style={[styles.dayName, { color: darkMode ? "#fff" : "#000" }]}
            >
              {getDayName(day)}
            </Text>
            <Text
              style={[styles.dayNumber, { color: darkMode ? "#bbb" : "#555" }]}
            >
              {day.getDate()}
            </Text>
          </View>
      ...
      <View style={styles.statusRow}>
        {weekDays.map((day, index) => {
          ...
          const { icon, color } = getStatusIcon(
            isFuture ? "not_updated" : dayStatus.status
          );
            ...
            >
              {isFuture ? (
                ...
                <View style={styles.statusCellContent}>
                  ...
                  {getStatusIcon(dayStatus.status).hasWarning && (
                    <Text style={styles.statusSymbol}>
                      {getStatusIcon(dayStatus.status).symbol}
                    </Text>
                  )}
                </View>
              )}
      ...
      <View style={styles.timeRow}>
        {weekDays.map((day, index) => {
          ...
          return (
            <View key={index} style={styles.timeCell}>
              <Text
                style={[styles.timeText, { color: darkMode ? "#bbb" : "#555" }]}
              >
                {shiftTime.start}
              </Text>
              <Text
